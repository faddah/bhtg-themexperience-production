	...
	var WPFormsConditionals = {
		...
		conditionalUpdateOptions: function(e, allFields) {
			...
			var fields     = $.extend({}, allFields),
				allowed    = ['text', 'textarea', 'select', 'radio', 'email', 'url', 'checkbox', 'number', 'payment-multiple', 'hidden'],
				changed    = [];
			...
			$('.wpforms-conditional-row').each(function(index, ele) {
				...
				if ( $fields.find('option[value="'+fieldSelected+'"]').length) {
					...
					if ($value.length && $value.is('select')) {
						...
						$value.append($('<option>', { text : wpforms_builder.select_choice }));
		...
		conditionalField: function(el, e) {
			...
			} else if ( data.field.type == 'select' || data.field.type == 'radio' || data.field.type == 'checkbox' || data.field.type == 'payment-multiple' ) {
