...
final class WPForms_Conditional_Logic {
					...
					foreach( $field['conditionals'] as $group_id => $group ) {
						foreach( $group as $rule_id => $rule ) {
							if ( !empty( $rule['field'] ) && !empty( $rule['operator'] ) && !empty( $rule['value'] ) ) {
								...
								if ( in_array( $form['fields'][$rule['field']]['type'] , array( 'select', 'checkbox', 'radio', 'payment-multiple' ) ) ) {

									if ( 'payment-multiple' == $form['fields'][$rule['field']]['type'] ) {
	...
	public function conditionals_block( $args = array(), $echo = true ) {
		...
		if ( !empty( $args['form'] ) ) {
			$form_fields = wpforms_get_form_fields( $args['form'], array( 'text', 'textarea', 'select', 'radio', 'email', 'url', 'checkbox', 'number', 'payment-multiple', 'hidden' ) );
		} else {
			$form_fields = array();
		}
			...
			if ( !empty( $group ) ) {
				...
				foreach( $group as $rule_id => $rule ) {
					...
					if (  in_array( $fields[$rule_field]['type'], array( 'text', 'textarea', 'email', 'url', 'number', 'hidden' ) ) ) {
						...
						if ( 'payment-multiple' == $fields[$rule_field]['type'] && isset( $fields[$rule_field]['value_raw'] ) && '' != $fields[$rule_field]['value_raw']  ) {
