...
class WPForms_Field_Payment_Multiple extends WPForms_Field {
	...
	public function field_options( $field ) {
		...
		// Choices option
		$tooltip = __( 'Add item choices for the form field.', 'wpforms' );
		...
		$choices = $this->field_element( 'label', $field, array( 'slug' => 'choices', 'value' => __( 'Items', 'wpforms' ), 'tooltip' => $tooltip ), false );
		...
		foreach ( $choices_values as $key => $value ) {
			...
			$choices .= '<li data-key="' . absint( $key ) . '">';
				...
				$choices .= sprintf( '<input type="text" name="fields[%s][choices][%s][value]" value="%s" class="value wpforms-money-input" placeholder="%s">', $field['id'], $key, $amount, $placeholder );
